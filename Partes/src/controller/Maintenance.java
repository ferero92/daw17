...
/**
 * Servlet implementation class Maintenance
 */
...
public class Maintenance extends HttpServlet {
		...
		try {
			...
			if(connect.isConnect()){
				...
				if(id == "-" || start == "" || end == null)
					array[1] = "report r, state s, users u:''::r.state&s.id AND r.user = u.id AND r.state <> 2 ORDER BY date DESC";
				else
					array[1] = "report r, state s, users u:user:"+id+":date BETWEEN '"+start+"' AND '"+end+"' AND r.state&s.id AND r.user = u.id AND r.state = 1 ORDER BY date DESC";
				...
				response.setHeader("refresh", "0;URL=pages/maintenance.jsp");
			}
			else
	...
	protected void error(HttpServletRequest request, HttpServletResponse response, String error) throws ServletException, IOException{
		...
		request.getSession().setAttribute("error", error+"&maintenance.jsp");
